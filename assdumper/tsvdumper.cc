...
// MPEG-TS uses 27Mhz clock. So in order to convertion a
// timestamp to msec, we need to do:
//
//   MSEC = TIMESTAMP / 27000000 * 1000
//
#define MSEC(u) ((u) / 27000)
    ...
    while (q < p + data_unit_loop_length) {
      ...
      if (data_unit_parameter == 0x20) {
        if (!prevsub.empty() && !(blank_p(prevsub) && prev_blank)) {
          printf("%lu\t%lu\t%s\n", MSEC(prevts - t0), MSEC(curts - t0), prevsub.c_str());
        }
